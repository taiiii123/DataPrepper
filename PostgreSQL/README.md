...
- PostgreSQL CLIツール（psql, pg_dump）がインストールされ、PATHが通っていること
- Windows環境
...
### 1. PostgreSQL CLI 確認
PostgreSQLコマンドラインツール（psql）が正しくインストールされているか確認します。

### 2. バックアップ実行
指定したデータベースの現在のデータを `pg_dump` でバックアップします。
...
```bash
psql "postgresql://ユーザー名:パスワード@ホスト名:ポート番号/データベース名" < ./output/backup/backup_YYYYMMDD.sql
```

**実行例**:
```bash
psql "postgresql://postgres:mypassword@localhost:5432/test_database" < ./output/backup/backup_20241224.sql
```
...
> **⚠️ セキュリティ**
> - パスワードは `./output/conf/pgpass.conf` ファイルに保存されます
> - 本番環境への誤登録にご注意ください
...
<details>
<summary><strong>PostgreSQL CLI ツールが見つからない場合</strong></summary>

- PostgreSQLがインストールされているか確認
- 環境変数PATHにPostgreSQLのbinフォルダが追加されているか確認
</details>
